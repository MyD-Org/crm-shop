...
import { actualizarProveedor, toProveedorDto } from "@/lib/cuotas-repo"
import { cuotasErrorResponse, NO_STORE } from "@/lib/cuotas-respuestas"
import { pingShopRevalidarCuotas } from "@/lib/shop-revalidar"

// PATCH /api/admin/cuotas/proveedores/[id] — edición parcial (incluye desactivar con { activo: false }).
// No hay DELETE de proveedores: se desactivan (sus escalones dejan de viajar al Shop).
...
export async function PATCH(req: Request, { params }: IdParams) {
  ...
  const r = await actualizarProveedor(guard.tenantId, id, body)
  ...
  return Response.json({ ok: true, propagado, proveedor: toProveedorDto(r.row) }, { headers: NO_STORE })
}
