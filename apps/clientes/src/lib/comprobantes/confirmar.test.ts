// Portado de apps/admin/src/lib/receipt-confirm.test.ts. Confirm con dependencias
// falsas (repo en memoria, R2 fake, aviso espía): orden de pasos, upload_missing libera
// el lease, 413s, re-sniff tras mutación del tmp, storage_error liberando, publicar 0
// filas ⇒ en curso sin mail, mail/delete del tmp que fallan no cambian el 200, errores
// de imagen ⇒ 415/422 (imagen.ts se mockea parcial; la conversión real se prueba en
// imagen.test.ts). Datos ficticios (tenant-a, dominios .example).

import { createHash } from "node:crypto";
import { describe, expect, it, vi } from "vitest";
import { confirmarComprobante, type RepoConfirmar } from "./confirmar";
import type { ComprobanteFila } from "./repo";
import { R2Error, R2TooLargeError, type R2Client } from "../r2";
import { claves } from "./claves";
import { MAX_FILE_BYTES } from "./validacion";

// normalizarImagen mockeado: image/heic ⇒ image_too_large (el resto pasa, no es lo que se
// prueba acá). ReceiptImageError y el resto del módulo quedan reales.
vi.mock("./imagen", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./imagen")>();
  return {
    ...actual,
    normalizarImagen: vi.fn(async (bytes: Uint8Array, mime: string) => {
...
const HEIC = (() => {
  ...
  b.set(
    Array.from("heic", (c) => c.charCodeAt(0)),
    8,
  );
  return b;
})();

function row(overrides: Partial<ComprobanteFila> = {}): ComprobanteFila {
  return {
    id: RECEIPT_ID,
    tenantId: TENANT.id,
...
interface RepoOptions {
  ...
  claimed?: ComprobanteFila | null;
  found?: ComprobanteFila | null;
  /** Default: fila `pending`. `null` = publish afectó 0 filas. */
  published?: ComprobanteFila | null;
}
...
function makeRepo(opts: RepoOptions = {}) {
  ...
  return {
    tomarParaConfirmar: vi.fn<RepoConfirmar["tomarParaConfirmar"]>(async () => claimed),
    buscarDelCliente: vi.fn<RepoConfirmar["buscarDelCliente"]>(async () => opts.found ?? null),
    liberar: vi.fn<RepoConfirmar["liberar"]>(async () => true),
    rechazar: vi.fn<RepoConfirmar["rechazar"]>(async () => {}),
    publicar: vi.fn<RepoConfirmar["publicar"]>(async () => published),
    // B: por defecto sin duplicado (el happy path de A no cambia).
    buscarDuplicado: vi.fn<RepoConfirmar["buscarDuplicado"]>(async () => null),
  };
}
...
function makeR2(opts: R2Options) {
  return {
    presignPut: vi.fn(async () => ({
      url: "https://r2.plataforma.example",
      headers: { "content-type": "application/pdf" },
      expiresAt: new Date(),
    })),
    presignGet: vi.fn(async () => "https://r2.plataforma.example"),
...
describe("confirmarComprobante", () => {
  it("happy path: verifica en orden, publica el buffer verificado y responde pending", async () => {
    ...
    const order = [
      ...
      repo.publicar.mock.invocationCallOrder[0],
    ...
    const finalKey = claves.final(TENANT.id, RECEIPT_ID, "pdf", NOW);
    ...
    expect(repo.publicar).toHaveBeenCalledTimes(1);
    const [publishFields, publishAt] = [repo.publicar.mock.calls[0]?.[2], repo.publicar.mock.calls[0]?.[3]];
    ...
    expect(r2.delete).toHaveBeenCalledWith(claves.tmp(TENANT.id, RECEIPT_ID));
  ...
  it("id que no es UUID ⇒ 404 not_found sin tocar repo ni R2", async () => {
    ...
    expect(result).toEqual({ ok: false, status: 404, code: "not_found", error: "No encontramos el comprobante." });
    expect(repo.tomarParaConfirmar).not.toHaveBeenCalled();
  ...
  it("claim 0 filas y fila inexistente/ajena ⇒ 404 not_found", async () => {
    ...
    expect(repo.buscarDelCliente).toHaveBeenCalledWith(TENANT.id, CLIENT.codigocliente, RECEIPT_ID);
    expect(r2.head).not.toHaveBeenCalled();
  });
  ...
  it("claim 0 filas y fila pending (re-confirm) ⇒ 200 idempotente, sin mail ni lectura de R2", async () => {
    ...
    expect(result).toEqual({ ok: true, status: "pending" });
    expect(r2.head).not.toHaveBeenCalled();
    expect(repo.publicar).not.toHaveBeenCalled();
    expect(deliver).not.toHaveBeenCalled();
  });
  ...
  it("claim 0 filas y processing con lease vivo ⇒ 202 in_progress", async () => {
    const { r2, deliver, run } = setup({
      claimed: null,
      found: row({ status: "processing", processingStartedAt: NOW }),
    });
    const result = await run();
  ...
  it("claim 0 filas y rejected ⇒ responde el MISMO código del rechazo (no lo resucita)", async () => {
    const { r2, deliver, run } = setup({
      claimed: null,
      found: row({ status: "rejected", rejectReason: "size_mismatch" }),
    });
    const result = await run();
  ...
  it("HEAD 404 ⇒ libera el lease (processing→uploading) y responde 409 upload_missing", async () => {
    ...
    expect(repo.liberar).toHaveBeenCalledWith(TENANT.id, RECEIPT_ID, NOW);
  ...
  it("HEAD con tamaño > 20 MiB ⇒ reject file_too_large (413) y borra el tmp", async () => {
    const { repo, r2, run } = setup(
      {},
      { head: { size: MAX_FILE_BYTES + 1, contentType: "application/pdf", etag: null } },
    );
    const result = await run();
    expect(result).toMatchObject({ ok: false, status: 413, code: "file_too_large" });
    expect(repo.rechazar).toHaveBeenCalledWith(TENANT.id, RECEIPT_ID, "file_too_large", NOW);
    expect(r2.delete).toHaveBeenCalledWith(claves.tmp(TENANT.id, RECEIPT_ID));
  ...
  it("HEAD con tamaño distinto al declarado ⇒ reject size_mismatch (413)", async () => {
    ...
    const result = await run();
    expect(result).toMatchObject({ ok: false, status: 413, code: "size_mismatch" });
    expect(repo.rechazar).toHaveBeenCalledWith(TENANT.id, RECEIPT_ID, "size_mismatch", NOW);
    expect(r2.getObject).not.toHaveBeenCalled();
  });
  ...
  it("range que no sniffea (SVG disfrazado) ⇒ reject unsupported_type (415) y borra el tmp", async () => {
    ...
    expect(result).toMatchObject({
      ...
      // CMP-1 "archivo falso": el mensaje que ve el cliente.
      error: "El archivo no es un comprobante válido. Suba un PDF o una imagen.",
    });
    expect(repo.rechazar).toHaveBeenCalledWith(TENANT.id, RECEIPT_ID, "unsupported_type", NOW);
    expect(r2.getObject).not.toHaveBeenCalled();
  ...
  it("image_too_large del procesamiento ⇒ reject 415 sin publish", async () => {
    ...
    expect(repo.rechazar).toHaveBeenCalledWith(TENANT.id, RECEIPT_ID, "image_too_large", NOW);
    expect(r2.put).not.toHaveBeenCalled();
  });

  it("processing_failed del procesamiento ⇒ reject 422 sin publish", async () => {
    const { normalizarImagen, ReceiptImageError } = await import("./imagen");
    vi.mocked(normalizarImagen).mockRejectedValueOnce(new ReceiptImageError("processing_failed"));
    const { repo, r2, run } = setup(
      { claimed: row({ status: "processing", declaredContentType: "image/heic", declaredSize: HEIC.length }) },
      { head: { size: HEIC.length, contentType: "image/heic", etag: null }, getRange: HEIC, getObject: HEIC },
    );
    ...
    expect(repo.rechazar).toHaveBeenCalledWith(TENANT.id, RECEIPT_ID, "processing_failed", NOW);
    expect(r2.put).not.toHaveBeenCalled();
  });
